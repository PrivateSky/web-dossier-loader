...
let areServiceWorkersEnabled = null;
...
const NavigatorUtils = {
  ...
  getAreServiceWorkersEnabled: (callback) => {
    if (areServiceWorkersEnabled !== null) {
      return callback(null, areServiceWorkersEnabled);
    }

    NavigatorUtils.getMobileConfig((err, config) => {
      areServiceWorkersEnabled = config == null;
      callback(null, areServiceWorkersEnabled);
    });
  },
  ...
  getMobileConfig: (callback) => {
    if (mobileConfig) {
      return callback(mobileConfig);
    }

    fetch("./mobile-config.json")
      .then((response) => response.json())
      .then((config) => {
        mobileConfig = config || {};
      ...
      .catch((err) => {
        console.log("Cannot load mobile-config.json", err);
        callback();
      });
  },
  ...
  getCanUseServiceWorkers: (callback) => {
    NavigatorUtils.getAreServiceWorkersEnabled((err, areServiceWorkersEnabled) => {
      callback(null, areServiceWorkersEnabled && NavigatorUtils.areServiceWorkersSupported());
    });
  },
  ...
  addServiceWorkerEventListener: (eventType, callback) => {
    getCanUseServiceWorkers((err, canUseServiceWorkers) => {
